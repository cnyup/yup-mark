...
| mermaid | **flowchart 子集 → ASCII 字符画**（盒子/菱形/边标签/檐列回边，D18）；其余类型占位框 `▶ mermaid · 类型 · N 行` | 光标进入显源码（字符画与源码同 D14 语义） |
...
## 12g. 落地备忘：整屏闪烁 + 状态栏残影修复（2026-09-11，用户实测反馈两连）

> 症状一：光标一动整屏闪一下。症状二（次日反馈）：连续移动后状态栏堆叠残影、整帧阶梯错位。

1. **闪烁根因（帧层）**：ink 6.8 对「帧高 ≥ 终端行数」的满屏应用**绕过增量 renderer**（ink.js `lastOutputHeight >= rows` 分支），每帧 `clearTerminal`（`\x1b[2J\x1b[3J\x1b[H`）+ 整帧重写——清屏瞬间即闪烁；其 `\x1b[?2026`（同步输出）包裹在 ConPTY/WSL interop 路径不透传，救不回来。**修复**：run-cli.mjs 的 esbuild 插件 `patch-ink-fullscreen` 锚点替换放行增量路径；**MT5 发布打包必须带上本插件**。
2. **残影根因（帧层）**：非满屏帧（帧高略小于终端行数，如浮层出现/窗口较高时）会被 ink 追加**尾随换行**，而增量 renderer 的回退光标计算（`cursorUp(N-1)`、`returnToBottom`）全部按「无尾随换行、光标停在最后一行」假设——**每帧整体下移一行**，状态栏逐帧堆叠。**修复**：同一补丁把 `isFullscreen` 判定恒置为 `isTTY`（帧永不带尾随换行，增量假设恒成立）。
3. **验证**：修复前捕获（YUPMARK_DEBUG_OUT 落盘 VT 载荷，当时尚在的调试设施）：每次按键 = 2J 清屏整帧重写 + 尾随 `\n` 后 `cursorUp(N-1)` 错一行。修复后目检：12↓+6↑+PgDn+8↓ 共 21 键，状态栏单行、无堆叠、表格网格无错位、行号与按键数吻合。
4. **附带修复**：Autosaver 原经 `session.subscribe` 订阅（每次 dispatch 触发，含纯选区移动）+ 文件基线初始化为空串 → 任何首次按键即"未保存"+800ms 后重写文件。改为 `session.subscribeDoc`（多播，仅 `tr.docChanged`）+ 基线 = 打开时内容。
5. **教训（工具链）**：run-cli.mjs 是 node 直接执行的 `.mjs`，**不能写 TypeScript 类型注解**（`Array<[string,string]>` 曾导致模块加载即崩，连续误诊三轮）；调试脚本一律 Write 工具落盘（heredoc 会吞正则反斜杠，两次踩坑）；调试用 capture 设施（改写 stdout）在退出路径有崩溃风险，已整体撤除——将来取证换 ink `onRender` 钩子或更成熟的方案。eslint ignores 增加 `spikes/**`（用户实验区构建产物不进产品 lint）。
6. 验证基线：36 文件 / 280 用例、typecheck×3、lint 全绿。
