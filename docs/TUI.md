...
>
> 对应 ADR：DESIGN.md D13–D18（用户已逐项敲定，2026-09-11）。
...
| D18 | mermaid 渲染 | **flowchart 子集 ASCII 字符画**（路线 B：零新依赖、全终端通用）；其余类型/不支持语法/超限 → 占位框降级；图形协议真渲染列 v2 |
...
| mermaid | **flowchart + sequenceDiagram 子集 → ASCII 字符画**（盒子/菱形/边标签/檐列回边；时序图=参与者盒+生命线+四类箭头+Note+自环，D18）；其余类型占位框 `▶ mermaid · 类型 · N 行` | 光标进入显源码（字符画与源码同 D14 语义） |
...
| 脚本组缺映射字符 | `^{i\pi}` → `^(iπ)`、`^\infty` → `^(∞)` | **平排回退**（组形式才回退；单字符脚本如 `x_@` 仍 null 显源码）；间距命令吸收前导空格防双空格 |
...
## 12f. MT4.5 落地备忘：mermaid flowchart 字符画（2026-09-11，D18 路线 B）

> 起因：用户实测发现 mermaid 只显占位框（"无法显示"）。问询 A（图形协议真图）/B（ASCII 字符画）/C（保持现状）未获显式选择，用户以"继续"放行——按推荐路线 B 先行（零依赖、全终端可见；A 需 ~150MB Chromium 且多数终端不支持，留 v2）。

1. **模块**：`packages/tui/src/mermaid-flowchart.ts`（纯函数，仅依赖 measure/preview 类型）。入口 `renderMermaidFlowchart(code, width): RenderSegment[][] | null`，null 即回落占位框（**绝不画错图**原则）。
2. **解析子集**：`flowchart/graph + TD/TB/LR`（BT/RL→null）；形状 `[..] (..) {..} ([..]) ((..)) [[..]] [(..)]` 与裸 id（渲染按 mermaid 默认=矩形）；边 `--> ---> --- -.- -.-> ==> ===> --o --x`、管道标签 `-->|是|`、内联标签 `-- 是 -->`、链式语句；引号标签可含括号（扫描跳过引号段）；HTML 实体解码；`%%` 注释；classDef/class/style/click/linkStyle 行忽略。subgraph/未识别语法→整体 null。
3. **布局**：DFS 回边检测（含自环）→ 去回边后最长路径分层 → 重心法层内排序（4 趟减交叉，稳定排序保声明序）。TD：相邻层正向边走层间通道（|Δcx|≤1 取中线直落，多入边自然并到同列）；跨层正向边与回边走右侧檐列（避免穿中间层节点）；同层边右直连/绕底回折；自环右侧绕底 ↑ 回底边。LR 为镜像（层=列，檐列改底部檐行，标签置连线上方）。边标签放不下时移到箭头行右侧（宁缺毋滥：putLabel 只写入空格/线段格，跨到框/箭头即整体放弃）。
4. **画布**：按显示列索引（CJK 宽字符 1 格 + 续格标记，复用 measure）；线段格方向位并集合并（`─`+`│`→`┼` 等 16 字符映射）；框/箭头/标签不可被线覆盖。栅格化收敛同色段并去尾随空格。
...
## 12f+. MT4.6 落地备忘：sequenceDiagram 字符画（2026-09-12，D18 扩展）

> 起因：用户实测 m3 第二个 mermaid 块（时序图）只显占位框。D18 路线 B 首期只实现了 flowchart 解析器，时序图属设计内降级；本里程碑补齐。

1. **模块**：`packages/tui/src/mermaid-sequence.ts`（纯函数）。入口 `renderMermaidSequence(code, width)`；layout.ts 按 flowchart → sequence → 占位框三级回落。
2. **解析子集**：participant/actor（as 别名，含 CJK）；消息 `->>`（实线）、`-->>`（虚线）、`-x`/`--x`（叉头）、`-)`/`--)`（开箭头）、`->`/`-->`（按开箭头）；`+`/`-` 激活前缀两侧剥除，activate/deactivate 行忽略（平铺渲染，不画激活框）；Note over X[,Y] / left of X / right of X；autonumber/loop/alt/opt/par/else/end/rect/box/critical/break 行忽略（块内平铺）；title 顶部淡显行；%% 注释。未知语法/超 8 参与者/超 80 条目/超终端宽/超 60 行 → 整体 null 占位框（绝不画错图）。
3. **布局**：参与者按声明/首现序排一行（居中标签盒 + 生命线），相邻间隙 = 跨该间隙最宽消息标签 + 4（over 双点 Note +6）；消息占 2 行（标签行居中于跨度 + 箭头行，箭头盖过起点生命线段、头部落在目标生命线上）；自环右绕回勾 2 行；Note 盒 3 行（left 首参与者左侧不足时左缘钳 0，右缘放宽到终端宽）；画布字符格覆盖式（生命线先铺，Note/箭头/标签后画）。
4. 验证：解析（声明/四类箭头/结构行忽略/Note 三定位/未知语法 null）+ 渲染（m3 样例/宽度契约/自环/Note 跨点/title/降级）**11 新用例**；mt2-layout 占位框断言改用 pie（时序图升级为字符画）；基线 **38 文件 / 302 用例**全绿，typecheck×3 + lint 零告警，m3 样例经 run-cli.mjs 真实管线目检。

## 12g. 落地备忘：整屏闪烁 + 状态栏残影/整帧重影修复（2026-09-11/12，用户实测反馈三连）

> 症状一：光标一动整屏闪。症状二：连续移动后状态栏逐帧堆叠。症状三：最大化+滚动+表格操作后状态栏残影大面积穿插内容中间、内容一行错位重影。且用户反馈状态栏信息太多太杂。

1. **闪烁根因（帧层）**：ink 6.8 对「帧高 ≥ 终端行数」的满屏应用**绕过增量 renderer**，每帧 `clearTerminal` + 整帧重写——清屏瞬间即闪烁；`\x1b[?2026`（同步输出）在 ConPTY/WSL interop 不透传救不回来。**修复**：esbuild 插件放行增量路径。
2. **残影/重影真正根因（帧层，症状二与三同源）**：ink 增量 renderer 是**光标相对寻址**——按「上一帧结束后光标停在帧底」假设做 `cursorUp(N-1)`/`cursorNextLine`/`returnToBottom` 相对移动。但 (1) 帧的可见行数随内容变化（yoga 把空 Box 折叠，帧高≠固定行数），(2) 相对移动在屏幕边缘被钳制（以 #5→#8 载荷回推可得光标 y=-12 的荒谬记账），(3) resize 后全行变化——三者叠加使记账与真实光标脱节，**逐帧累积成整帧漂移**：内容一行错位重影 + 旧帧底行（状态栏）残留堆叠。取证通道：`YUPMARK_DEBUG_OUT=<文件>`（cli.tsx，proxy stdout 传给 ink，不触碰 process.stdout）+ `WSLENV` 传变量。
