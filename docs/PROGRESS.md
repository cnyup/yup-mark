...
**验证基线**：typecheck 0 错误（node/web/tui 三 project）· eslint 干净 · vitest 30 文件 / 221 用例全绿。
...
- **表格交互现状**：网格只读渲染 + 光标进入显源码编辑（完整可用）；**网格编辑模式（§5 原设计）待用户定夺**——TUI.md §12b-4 列了两个选项。
- **非 TTY 预览升级**：cli CI 输出走完整装配管线，samples/m1、m3 黄金样例经此验证（含 `∑ᵢ₌₁ⁿ` 近似与 `^\infty` 降级路径）。
- **验证**：typecheck×3 / lint / **221 用例**（+24 MT2 用例）；每键 12.3ms@10k 行无回归。
- **下一步**：用户定表格交互方向 → MT3（文件树/大纲/多标签/查找替换/视图三件套/上下文菜单）。
